...
The scripts live at [`install-mcp.sh`](./install-mcp.sh) and [`install-mcp.ps1`](./install-mcp.ps1) if you want to read them first. The Linux/macOS script installs both `fff-mcp` and `fffq` from the latest GitHub release, with a source-build fallback for platforms without a prebuilt asset.
