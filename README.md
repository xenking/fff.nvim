...
The script lives at [`install-mcp.sh`](./install-mcp.sh) if you want to read it first. It installs both `fff-mcp` and `fffq` from the latest GitHub release, with a source-build fallback for platforms without a prebuilt asset.
